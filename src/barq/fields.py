...
class Field:
    ...
    def __init__(self, attr=None, call=False, label=None, required=True, default=None):
        self.attr = attr
        self.call = call
        self.label = label
...
class DateField(Field):
    ...
    def to_value(self, value: str) -> str:
        if value:
            return datetime.strptime(value, self.date_format).date().isoformat()
...
class SelfField(Field):
    def as_getter(self, serializer_field_name, serializer_cls):
        attr = self.attr
        if attr is None:
            attr = serializer_field_name
        return partial(getattr, serializer_cls, attr)
